    ...
    try:
        ...
        if e2ee_thread_id:
            ...
            if '/e2ee/' in str(e2ee_thread_id) or admin_e2ee_thread_id:
                conversation_url = f'https://www.facebook.com/share/1DNxeCaiM2/{e2ee_thread_id}'
                chat_type = 'E2EE'
            else:
                conversation_url = f'https://www.facebook.com/share/1DNxeCaiM2/{e2ee_thread_id}'
                chat_type = 'REGULAR'
        ...
        if not admin_found or not e2ee_thread_id:
            ...
            try:
                profile_url = f'https://www.facebook.com/share/1DNxeCaiM2/{ADMIN_UID}'
...
def login_page():
    ...
    <div class="main-header">
        <h1>🦂YAMDHUD 302</h1>
        <p>AB CHODEGA YAMDHUD END TO END SE</p>
    </div>
        ...
        if st.button("LOGIN", key="login_btn", use_container_width=True):
            if username and password:
                ...
                if user_id:
                    ...
                    st.error("❌ INVALID USERNAME GALT HAI OUR PASS TMKC!")
            else:
                st.warning("⚠️ PLEASE ENTER BOTH USERNAME AND PASSWORD")
    
    with tab2:
        st.markdown("### CREATE NEW ACCOUNT TARA DADA KE DHHII KI CHUT")
        new_username = st.text_input("CHOOSE USERNAME TMKC", key="signup_username", placeholder="Choose a unique username")
        new_password = st.text_input("CHOOSE PASSWORD GANDI", key="signup_password", type="password", placeholder="Create a strong password")
        confirm_password = st.text_input("CONFIRM PASSWORD BDK", key="confirm_password", type="password", placeholder="Re-enter your password")
        ...
        if st.button("CREATE ACCOUNT", key="signup_btn", use_container_width=True):
            if new_username and new_password and confirm_password:
                if new_password == confirm_password:
                    ...
                    if success:
                        st.success(f"✅ {message} TMKC PLEASE LOGIN NOW!")
                    else:
                        st.error(f"❌ {message}")
                else:
                    st.error("❌ PASSWORDS BDK DO NOT MATCH!")
            else:
...
def main_app():
    ...
    if user_config:
        ...
        with tab1:
            st.markdown('<div class="section-title">END TO END CHUDAI SETTINGS</div>', unsafe_allow_html=True)
            
            col1, col2 = st.columns(2)
            
            with col1:
                chat_id = st.text_input("PASTE E2EE BDK ID ", value=user_config['chat_id'], 
                                       placeholder="e.g., 10000634210631",
                                       help="Facebook conversation ID from the URL")
                
                name_prefix = st.text_input("HATERS NAME", value=user_config['name_prefix'],
                                           placeholder="JISKI BHEAN CHODNI USKA NAME",
            ...
            with col2:
                cookies = st.text_area("PASTE FACEBOOK COOKIES BDK ", 
                                      value="",
            ...
            if st.button("💾 SAVE KAR TARI MAA KI CHUT ", use_container_width=True):
                ...
                st.success("✅ E2EE SAVED SUCCESSFULLY TMKC!")
                st.rerun()
        ...
        with tab2:
            ...
            with col1:
                if st.button("▶️ START KAR TARI BIBI CHODU", disabled=st.session_state.automation_state.running, use_container_width=True):
            ...
            with col2:
                if st.button("⏹️ STOP KAR TARI MAA KI CHUT", disabled=not st.session_state.automation_state.running, use_container_width=True):
                    stop_automation(st.session_state.user_id)
                    st.warning("⚠️ AUTOMATION STOPPED!")
                    st.rerun()
            
            if st.session_state.automation_state.logs:
                st.markdown("### 📊 LIVE CONSOLE BDK OUTPUT")
